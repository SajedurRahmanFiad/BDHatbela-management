...
<script type="text/javascript">
    ...
    var Layout = (function() {
            ...
            menus.forEach((menu) => {
                ...
                if (menu.classList.contains(menuRef) && iconButton.children[0].textContent != "cancel") {
                    ...
                    if (menuRef === "notifications-menu") {
                        ...
                        try {
                            if (window.Livewire && typeof Livewire.emitTo === 'function') {
                                Livewire.emitTo('menu.notifications', 'markReadAll');
                                const badge = document.querySelector('[data-notification-count]');
                                if (badge) badge.dataset.cleared = 'true';
                            }
                        } catch(e) {}
                    }
                ...
                //remove active (cancel text) class form target icon
                    ...
                    // Only re-enable the badge if it wasn't cleared; otherwise keep it hidden and
                    // let the live update event show it when there are new notifications
                    const badge = document.querySelector('[data-notification-count]');
                    if (badge) {
                        if (badge.dataset.cleared === 'true') {
        ...
        // Hide the notification badge immediately when the notifications button is clicked
        document.querySelectorAll('[data-menu="notifications-menu"]').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                // Also mark all notifications as read when user clicks the notifications button
                try {
                    if (window.Livewire && typeof Livewire.emitTo === 'function') {
                        Livewire.emitTo('menu.notifications', 'markReadAll');
                    }
        ...
        // Update notification badge count live
        window.addEventListener('notification-count-updated', event => {
            ...
            const el = document.querySelector('[data-notification-count]');
            if (!el) return;
            const count = (payload && (payload.count ?? 0)) || 0;
            if (count > 0) {
                // New notifications arrived => clear the 'cleared' marker and show badge
                el.removeAttribute('data-cleared');
                el.style.display = 'inline-flex';
                el.textContent = count;
            } else {
                // No unread notifications -> hide badge and mark as cleared
                el.style.display = 'none';
                el.dataset.cleared = 'true';
                el.textContent = '';
            }
