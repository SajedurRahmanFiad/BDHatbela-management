...
class Transactions extends Controller
{
    ...
    public function index()
    {
        ...
        $transactions = Transaction::with('account', 'category', 'contact', 'taxes', 'document.items.item')->collect(['paid_at'=> 'desc']);
        ...
        // Include invoice shipping (once per invoice) into expense totals so profit reflects shipping only once.
        $processed_docs = [];
        foreach ($transactions as $transaction) {
            ...
            if (in_array($doc->id, $processed_docs)) {
                continue;
            }
            ...
            if ($shipping > 0) {
                ...
                $totals['expense'] += $shipping_converted;
            }

            $processed_docs[] = $doc->id;
        }
