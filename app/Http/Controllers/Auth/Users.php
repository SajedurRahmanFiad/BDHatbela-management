...
class Users extends Controller
{
    ...
    {
        // If this is a JSON request coming from the search/filter widget (e.g. route defined as users.index with a search filter),
        // return a simplified users list suitable for populating select/dropdown options.
        if (request()->expectsJson() && request()->has('search')) {
            $search = request('search');
            $enabled = $this->getSearchStringValue('enabled', '', $search);

            $query = user_model_class()::query()->isNotCustomer();

            if ($enabled !== '') {
                $query->where('enabled', (int) $enabled);
            }

            $users = $query->take(setting('default.select_limit', 100))->get()->sortBy('name')->values();
        ...
        $users = user_model_class()::with('companies', 'media', 'permissions', 'roles')
            ->where(function ($query) {
                // Show all internal users that can access the admin panel
                // plus any users that have the 'employee' role, even if they
                // don't have the 'read-admin-panel' permission.
                $query->isNotCustomer()
    ...
    public function create()
    {
        ...
        $roles = role_model_class()::all()->reject(function ($r) {
            ...
            // Ensure the 'employee' role is not rejected here so admins can assign it when creating users
            if ($r->name == 'employee') {
                $status = false;
            }

            // Ensure the 'admin' role is not rejected and remains selectable
            if ($r->name == 'admin') {
                $status = false;
            }

            // Exclude 'manager' and 'accountant' roles from the create user form
            if (in_array($r->name, ['manager', 'accountant'])) {
                $status = true;
            }
        ...
        // Ensure admin role is present in the list when available
        $adminRole = role_model_class()::where('name', 'admin')->first();
        if ($adminRole && ! $roles->has($adminRole->id)) {
            $roles->put($adminRole->id, $adminRole->display_name);
        }
    ...
    public function edit($user_id)
    {
        ...
        if ($user->isCustomer()) {
            ...
            $roles = role_model_class()::all()->reject(function ($r) {
                ...
                // Ensure the 'employee' role is not rejected here so admins can assign it when editing users
                if ($r->name == 'employee') {
                    $status = false;
                }
                ...
                // Exclude 'manager' and 'accountant' roles from the edit user form
                if (in_array($r->name, ['manager', 'accountant'])) {
                    $status = true;
                }
