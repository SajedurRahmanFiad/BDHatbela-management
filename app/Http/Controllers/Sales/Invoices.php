...
class Invoices extends Controller
{
    ...
    public function index()
    {
        ...
        $invoices = Document::invoice()->with('contact', 'items', 'items.taxes', 'item_taxes', 'last_history', 'transactions', 'totals', 'histories', 'media', 'owner')->collect(['document_number'=> 'desc']);
    ...
    /**
     * Mark the invoice as picked.
     *
     * @param  Document $invoice
     *
     * @return Response
     */
    public function markPicked(Document $invoice)
    {
        event(new \App\Events\Document\DocumentMarkedPicked($invoice));

        $message = trans('documents.messages.marked_as', ['type' => trans_choice('general.invoices', 1), 'status' => trans('documents.statuses.picked')]);
    ...
    /**
     * Add invoice to Steadfast courier.
     *
     * @param  Document $invoice
     *
     * @return Response
