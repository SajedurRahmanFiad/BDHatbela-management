...
class Document extends FormRequest
{
    ...
    /**
     * Prepare the issued_at and due_at fields for validation by normalizing
     * times so same-day edits don't fail due to time-of-day differences.
     */
    protected function prepareForValidation()
    {
        if ($this->has('issued_at')) {
            try {
                $issuedAt = Date::parse($this->request->get('issued_at'))->startOfDay()->format('Y-m-d H:i:s');
                $this->merge(['issued_at' => $issuedAt]);
            } catch (\Exception $e) {
                // If parsing fails, let validation handle the invalid value
            }
        }

        if ($this->has('due_at')) {
            try {
                $dueAt = Date::parse($this->request->get('due_at'))->endOfDay()->format('Y-m-d H:i:s');
                $this->merge(['due_at' => $dueAt]);
            } catch (\Exception $e) {
                // If parsing fails, let validation handle the invalid value
            }
        }
