...
class Notifications extends Component
{
    ...
    public function markRead($type, $notification_id, $message = true)
    {
        ...
        if (! $message) {
            // still update unread count silently
            $count = user() ? user()->unreadNotifications->count() : 0;
            $this->dispatch('notification-count-updated', ['count' => $count]);
        ...
        $this->dispatch('mark-read', ['type' => 'notifications', 'message' => trans('notifications.messages.mark_read', ['type' => $type])]);
    ...
    public function markReadAll()
    {
        ...
        $this->dispatch('mark-read-all', ['type' => 'notifications', 'message' => trans('notifications.messages.mark_read_all', ['type' => trans('general.export')])]);
    ...
    public function markNewAppRead($notification_id)
    {
        ...
        // update unread count in case it changed
        $count = user() ? user()->unreadNotifications->count() : 0;
        $this->dispatch('notification-count-updated', ['count' => $count]);
    }
