...
abstract class Index extends Component
{
    ...
    public function getSummaryItems($type, $summaryItems)
    {
        ...
        foreach ($totals as $key => $total) {
            // Hide specific totals for employee users
            if (user() && user()->isEmployee()) {
                $skipKeys = ['overdue', 'open', 'picked', 'draft'];
    ...
    public function getTabSuffix($type, $tabSuffix)
    {
        ...
        $picked    = str_replace('status:', '', config('type.document.' . $type . '.route.params.picked.search'));
        $completed = str_replace('status:', '', config('type.document.' . $type . '.route.params.completed.search'));
        $cancelled = str_replace('status:', '', config('type.document.' . $type . '.route.params.cancelled.search'));
