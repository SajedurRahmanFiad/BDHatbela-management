...
class ShowInNeww
{
    ...
    public function handle(Event $event)
    {
        ...
        $title = user()->isEmployee() ? trim(trans_choice('general.orders', 1)) : trim(trans_choice('general.invoices', 1));
        if ($this->canAccessMenuItem($title, 'create-sales-invoices') || user()->isEmployee()) {
        ...
        if (! user()->isEmployee() && $this->canAccessMenuItem($title, 'create-purchases-vendors')) {
