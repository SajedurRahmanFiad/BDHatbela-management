...
class ShowInSettings
{
    ...
    public function handle(Event $event)
    {
        ...
        $title = 'Courier';
        if ($this->canAccessMenuItem($title, 'read-admin-panel')) {
            $menu->route('settings.courier.edit', $title, [], 110, ['icon' => 'local_shipping', 'search_keywords' => 'courier steadfast delivery']);
        }
    }
}
