...
class ShowInAdmin
{
    ...
    public function handle(Event $event)
    {
        ...
        if ($this->canAccessMenuItem($title, ['read-sales-invoices', 'read-sales-customers']) || user()->isEmployee()) {
            $menu->dropdown($title, function ($sub) use ($attr) {
                $title = trim(trans_choice('general.invoices', 2));
                if ($this->canAccessMenuItem($title, 'read-sales-invoices') || user()->isEmployee()) {
        ...
        if (!user()->isEmployee() && $this->canAccessMenuItem($title, ['read-purchases-bills', 'read-purchases-vendors'])) {
            $menu->dropdown($title, function ($sub) use ($attr) {
