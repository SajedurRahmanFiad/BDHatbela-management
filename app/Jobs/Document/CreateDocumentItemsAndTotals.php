...
class CreateDocumentItemsAndTotals extends Job implements HasOwner, HasSource, ShouldCreate
{
    ...
    public function handle(): void
    {
        ...
        // Add shipping fee if sent directly as 'shipping' (some frontends post only 'shipping' instead of totals[])
        $docShipping = null;

        if (! empty($this->request['shipping'])) {
            $shippingAmount = round(abs($this->request['shipping']), $precision);

            // Avoid duplicating if totals[] already contains shipping
            $hasShipping = false;
            ...
            // Set document shipping from explicit param (if totals[] does not already provide it)
            $docShipping = $shippingAmount;
            ...
            if (! $hasShipping && $shippingAmount > 0) {
                ...
                $this->request['amount'] += $shippingAmount;
        ...
        if (! empty($this->request['totals'])) {
            foreach ($this->request['totals'] as $total) {
                ...
                // If this total is a shipping total, capture it so we also store it on the document
                if (! empty($total['code']) && $total['code'] === 'shipping') {
                    $docShipping = $total['amount'];
                }
        ...
        // Persist shipping value on the document (so shipping is available without querying totals)
        if ($docShipping !== null) {
            $this->document->shipping = $docShipping;
            $this->document->save();
        }
    }
