...
class CreateDocumentItem extends Job implements HasOwner, HasSource, ShouldCreate
{
    ...
    public function handle(): DocumentItem
    {
        ...
        // Persist purchase price snapshot on document item to avoid historical COGS changing
        if (! isset($this->request['purchase_price'])) {
            $purchase_price = 0;
            if ($item_id) {
                $itemModel = \App\Models\Common\Item::find($item_id);
                $purchase_price = $itemModel->purchase_price ?? 0;
            }

            $this->request['purchase_price'] = round($purchase_price, $precision);
        } else {
            $this->request['purchase_price'] = round($this->request['purchase_price'], $precision);
        }
